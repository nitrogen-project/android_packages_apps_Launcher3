...
import com.android.launcher3.icons.SimpleLauncherIcons;
...
public class IconsHandler {
    ...
    public BitmapDrawable getBitmap(Context context, Drawable drawable) {
        if (drawable != null && drawable instanceof BitmapDrawable) {
            ...
            return new BitmapDrawable(context.getResources(), SimpleLauncherIcons.createIconBitmap(bm, context, mIconNormalizer, mShadowGenerator));
        }
        return null;
    }
    ...
    private Bitmap getDefaultAppDrawable(ComponentName componentName) {
        ...
        return generateBitmap(componentName, SimpleLauncherIcons.createIconBitmap(drawable, mContext, mIconNormalizer, mShadowGenerator));
    }
    ...
    private Bitmap generateBitmap(ComponentName componentName, Bitmap defaultBitmap) {
        ...
        if (mBackImages.isEmpty()) {
            return SimpleLauncherIcons.createBadgedIconBitmap(mIconNormalizer, mShadowGenerator, d, Process.myUserHandle(),
                    mContext, Build.VERSION.SDK_INT, true, isDefaultIconPack());
        }

        Bitmap wrapped = SimpleLauncherIcons.createBadgedIconBitmap(mIconNormalizer, mShadowGenerator, d, Process.myUserHandle(),
